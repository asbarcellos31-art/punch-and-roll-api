  ...
  try {
    ...
    if (cfgAus && cfgAus.valor === '1' && hora === 10 && dia !== ultimoDiaAusentes) {
      ...
      for (const a of candidatos) {
        ...
        const r = await enviarWA(a.tel, msg);
        await db.query('INSERT INTO wa_envios (nome,telefone,mensagem,tipo,status,erro) VALUES (?,?,?,?,?,?)',
          [a.nome, telFmt, msg, 'AUSENTE', r.sucesso?'ENVIADO':'ERRO', r.erro||null]);
        enviados++;
        await new Promise(x=>setTimeout(x,3000));
      }
      if (enviados) console.log(`[Ausentes] ${enviados} mensagens enviadas`);
    }
  } catch(e) { console.error('[Cron Ausentes]',e.message); }
...
app.post('/api/despesas', auth, adminOnly, async (req, res) => {
  try {
    ...
    // Recorrente: cria 12 meses à frente automaticamente ao cadastrar
    if (recorrente && n === 1) {
      const [y, m, d] = data_vencimento.split('-').map(Number);
      for (let i = 1; i <= 12; i++) {
        const proxDt = new Date(y, m - 1 + i, d);
        const proxVenc = `${proxDt.getFullYear()}-${String(proxDt.getMonth()+1).padStart(2,'0')}-${String(proxDt.getDate()).padStart(2,'0')}`;
        const [existente] = await db.query(
          'SELECT id FROM despesas WHERE descricao=? AND DATE_FORMAT(data_vencimento,"%Y-%m-%d")=? LIMIT 1',
          [descricao, proxVenc]
        );
        if (!existente.length) {
          await db.query(

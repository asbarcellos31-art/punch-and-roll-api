...
(function () {
  ...
  style.textContent = `
    #pwa-fab {
      ...
      border-radius: 50%;
      background: #d4111c;
      box-shadow: 0 4px 16px rgba(212,17,28,.55);
      border: none;
      cursor: pointer;
      display: flex;
      align-items: center;
    ...
    #pwa-fab:hover { transform: scale(1.08); box-shadow: 0 6px 24px rgba(212,17,28,.7); }
    #pwa-fab img { width: 36px; height: 36px; border-radius: 8px; pointer-events: none; }
    #pwa-fab-badge {
    ...
    btn.innerHTML = `
      ...
      <span id="pwa-fab-badge">
        <svg xmlns="http://www.w3.org/2000/svg" width="10" height="10" viewBox="0 0 24 24" fill="#d4111c">
          <path d="M19 9h-4V3H9v6H5l7 7 7-7zm-2 9H7v2h10v-2z"/>
        </svg>
      </span>
    `;
  ...
  function showTooltip(html, onClose) {
    const old = document.getElementById('pwa-tooltip');
    if (old) { old.remove(); return; }

    const tip = document.createElement('div');
    tip.id = 'pwa-tooltip';
    tip.innerHTML = `<button id="pwa-tooltip-close" aria-label="Fechar">✕</button>${html}`;
    document.body.appendChild(tip);
    document.getElementById('pwa-tooltip-close').onclick = () => { tip.remove(); if (onClose) onClose(); };
  }
  ...
  window.addEventListener('appinstalled', () => {
    ...
    if (fab) fab.remove();
    const tip = document.getElementById('pwa-tooltip');
    if (tip) tip.remove();
  });
  ...
  fab.onclick = async () => {
    ...
    if (deferredPrompt) {
      ...
      showTooltip(`
        <strong>Instale o app!</strong>
        <span>No seu navegador, toque em <strong style="color:#f9fafb">⋮ Menu</strong><br>
        depois em <strong style="color:#f9fafb">"Adicionar à tela inicial"</strong></span>
      `);
    }
  };
})();
